...
export const POST = withApi('write:themes', async (req) => {
  ...
  const project = await getActiveProject();
  const existing = await prisma.documentCategory.findFirst({ where: { slug: body.data.slug, projectId: project.id } });
  if (existing) return NextResponse.json({ error: 'Slug already exists' }, { status: 409 });

  const max = await prisma.documentCategory.findFirst({ where: { projectId: project.id }, orderBy: { order: 'desc' }, select: { order: true } });
