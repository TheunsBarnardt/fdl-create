...
export const POST = withApi('write:themes', async (req) => {
  ...
  const existing = await prisma.documentCategory.findUnique({ where: { slug: body.data.slug } });
  if (existing) return NextResponse.json({ error: 'Slug already exists' }, { status: 409 });

  const max = await prisma.documentCategory.findFirst({ orderBy: { order: 'desc' }, select: { order: true } });
