...
import { rangeToSince, statusClass, bucketize, percentile, formatDuration, type TimeRange } from '@/lib/logs';
...
export default async function AnalyticsPage({
  ...
  const errorCount = logs.filter((l) => l.status >= 500).length;
  ...
  const uniqueActors = new Set(logs.map((l) => l.tokenId ?? l.userId ?? 'anon')).size;

  const volume = bucketize(logs, range);
  ...
  for (const l of logs) statusBuckets[statusClass(l.status)]++;
  ...
  return (
    <section className="flex-1 flex flex-col overflow-hidden">
      <ScreenHeader
        title="Analytics"
        chips={<Chip tone="accent">{total} requests</Chip>}
        actions={
